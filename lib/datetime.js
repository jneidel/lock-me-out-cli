...
function isValidDate( ipt ) {
  ...
  if ( typeof ipt !== "string" ) {
    validationErrors.push( "date/time has to be a string" );
  }

  if ( ipt.split( " " ).length !== 1 && ipt.split( " " ).length !== 2 ) {
    validationErrors.push( "date/time has to many or to few values separated by spaces (max two: 1 date, 1 time)" );
  }
